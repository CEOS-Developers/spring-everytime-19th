...
-----------------------------------------------------
### dockerfile 작성 후 docker image 생성
![img_15.png](img_15.png)

#### docker images 확인
![img_16.png](img_16.png)

#### docker-compose.yaml 생성 후 실행
왜 docker-compose를 쓸까??? -> 여러서버를 사용할때 동시에 올리기위한 것일뿐
에러발생....?
![img_17.png](img_17.png)

내부에서 이미 사용하고 있기 떄문(근데 왜...?)

앞의 포트 (호스트의 포트): 호스트에서 해당 서비스에 접속할 때 사용하는 포트

뒤의 포트 (컨테이너의 포트): 컨테이너 내에서 실행 중인 서비스의 포트
...
성공~~~!
왜 둘다 접속이 가능할까? 두개다 열린거고 이떄 로드밸런싱이나 병렬처리가 가능하다~~


### 기존 VM

- OS위에서 가상화 소프트웨어를 사용해 하드웨어를 매뮬레이션해 게스트 운영체제를 생성
- 가상 머신은 물리적 호스트와 그 위에 설치되는 가상화 하이퍼바이저(서로 다른 복수개의 OS를 단일 물리 머신 위에서 스케줄링 가능한 소프트웨어)에서 작용

### Docker

도커를 사용할 시 인프라와 애플리케이션을 분리 가능 + 애플리케이션을 관리하는 것과 같은 방법으로 인프라를 관리
- 코드 작성과 실행(서비스 제공) 간의 지연을 줄일 수 있다
- 버전 충돌 문제 해결
- 운영 체제의 차이 -> 컴퓨터가 운영체제, 내부 프로그램과 상관 없이 독립된 환경을 만들고 싶을 때
#### Docker Image
- 애플리케이션과 그 애플리케이션을 실행하기 위해 필요한 모든 환경(설정 포함)을 묶어서 저장한 파일
- 이미지를 빌드할 때에는 빌드 단계에 필요한 명령어 순서, 환경 설정 등과 이미지를 실행하는 명령어를 Dockerfile에 저장해서 실행한다
- ex_ jar를 이미지로 만들고자 할 때에는 Dockerfile에 `./gradlew build`, `java-jar`를 실행하는 명령어를 순서대로 적어주어야한다
- 도커 이미지는 읽기 전용 파일이기 때문에 처음 빌드한 이후로 상태값이 변하지 않는다

#### Docker Container
- 어플리케이션과 바이너리파일(설치파일), 라이브러리를 묶어서 만든 하나의 독립적인 실행공간
- 실행중인 이미지
- 실행 후 변경 사항 (데이터베이스 변경, 서비스 추가 등등)은 컨테이너 안에 이미지와는 별도록 저장되어서 컨테이너를 삭제하면 함께 삭제된다
- 이미지는 레이어가 쌓인 형태
...
#### Docker Compose
- 명령어 한 번으로 여러 컨테이너를 키고 끌 수 있는 서비스
- ngnix 컨테이너, springboot 컨테이너, DB 컨테이너가 모두 각각 존재할 때 각각 컨테이너에 대해 RUN을 세번 수행하는 것이 아니라 도커 컴포즈를 이용해 한번에 킬 수 있다.

#### Docker Network
- Docker 컨테이너는 격리된 환경에서 돌아가기 때문에 기본적으로 다른 컨테이너와의 통신이 불가능
- 해결 : 여러 개의 컨테이너를 하나의 도커 네트워크에 연결시키면 서로 통신이 가능
- 동일한 도커 네트워크에 위치하면 localhost의 port없이 컨테이너끼리 서로 통신이 가능
  - 종류:
    - bridge : 하나의 호스트 컴퓨터 내에서 여러 컨테이너들이 서로 소통할 수 있도록 해줌, 실제 프로덕션 환경에 적합하다고 볼 수는 없고, 간단히 개발환경에서 테스트 해보기에 좋음
...
- 네트워크 생성
  ...
  - -d : 백그라운드에서 컨테이너가 실행되도록 함
    -p 8080:8080 : 호스트와 컨테이너의 포트를 연결(포워딩)
